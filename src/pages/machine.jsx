...
const Machine = () => {
  ...
  useGSAP(() => {
    gsap.to(imageDivRef.current, {
      scrollTrigger: {
        ...
        scrub: 1,
        pin:true,
        onUpdate:(elem) =>{                                //Image scrolling with gsap animations
          const imgindex = 
           Math.min( Math.floor(elem.progress * imgarray.length), imgarray.length - 1, );  
  ...
  return (
    ...
    <div className="bg-white min-h-screen w-full flex flex-col overflow-x-hidden ">
      <div
        ref={imageDivRef}
        className="  absolute  rounded-full top-[12vw] left-[38vw] h-[22vw] w-[22vw] overflow-hidden "
      >
        <img
          ref={imgRef}
